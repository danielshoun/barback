...
class PollController {
    ...
    fun createPoll(@PathVariable(name = "organizationId") organizationId: Long,
    ...
    fun submitPoll(@PathVariable(name = "organizationId") organizationId: Long,
    ...
    fun getCurrentPolls(@PathVariable(name = "organizationId") organizationId: Long,
                        @CookieValue(value = "session") session: String) : ResponseEntity<List<Poll>> {
        return pollService.getCurrentPolls(organizationId, session)
    }

    @GetMapping("/api/v1/organization/{organizationId}/poll/past")
    fun getPastPolls(@PathVariable(name = "organizationId") organizationId: Long,
                     @CookieValue(value = "session") session: String) : ResponseEntity<List<Poll>> {
        return pollService.getPastPolls(organizationId, session)
    }

    @GetMapping("/api/v1/organization/{organizationId}/poll/{pollId}/choices")
    fun getPollChoices(@PathVariable(name = "organizationId") organizationId: Long,
                       @PathVariable(name = "pollId") pollId: Long,
                       @CookieValue(value = "session") session: String) : ResponseEntity<List<PollChoice>> {
    ...
    fun checkVote(@PathVariable(name = "organizationId") organizationId: Long,
                  @PathVariable(name = "pollId") pollId: Long,
                  @RequestParam(name = "hash") voteHash: String,
                  @CookieValue(value = "session") session: String) : ResponseEntity<PollChoice> {
